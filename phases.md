...
- [ ] Add active-session awareness using explicit host activation/focus signals and a user-controlled session selector: `ghost session list`, `ghost session status`, and session selection. GhostD must never select a session merely because it is the most recently seen session from another workspace or provider.
